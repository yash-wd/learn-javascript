 ...
 *   assets/solution-sources.js — window.SOLUTION_SOURCES (raw worked-solution
 *                              source, so lesson.html can render each lesson's
 *                              "worked solution" in the themed viewer offline)
 *
...
const SOLUTIONS = join(LESSONS, 'solutions');
...
const OUT_SOLUTIONS = join(ROOT, 'assets', 'solution-sources.js');
...
// ── worked solutions: raw source, keyed by path, for the lesson viewer ──────
const solutions = {};
const solFiles = readdirSync(SOLUTIONS)
  .filter((f) => /^[0-9].*-solutions\.js$/.test(f))
  .sort();
for (const f of solFiles) {
  solutions['lessons/solutions/' + f] = readFileSync(join(SOLUTIONS, f), 'utf8');
}
...
writeFileSync(OUT_SOLUTIONS, banner + 'window.SOLUTION_SOURCES = ' + JSON.stringify(solutions) + ';\n');

console.log(
  `search index + sources + manifest + solutions: ${files.length} lessons, ${solFiles.length} solutions → ` +
  `assets/search-index.js, assets/lesson-sources.js, assets/lessons-manifest.js, assets/solution-sources.js`
);
